...
interface HomeViewProps {
  /** Rendered on the server so the client does not refetch it on mount. */
  initialInfo: InfoResponse;
  initialPosts: PostsResponse;
  initialBlogPosts: BlogPostWithAuthor[];
}
...
export default function HomeView({
  ...
  const hasNoFilters =
    selectedCategory === null && selectedStatus === null && searchBox === "";
  ...
  const infoQuery = useInfoQuery(selectedStatus, initialInfo);
  ...
  const query = usePostsQuery(
    ...
    // Only the unfiltered first page matches what the server rendered; any
    // filter in the URL is a different query key and must be fetched.
    hasNoFilters ? initialPosts : undefined
  );

  const { data: blogPosts = [], isPending: blogLoading } = useQuery<
    BlogPostWithAuthor[]
  >({
    queryKey: ["blog-posts"],
